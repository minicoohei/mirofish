...
Wraps graph search, node reading, edge querying tools for Report Agent

Core retrieval tools (optimized):
1. InsightForge (deep insight retrieval) - Most powerful hybrid retrieval, auto-generates sub-questions
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """Convert to text format (full version, untruncated)"""
        text_parts = [
        ...
        if self.active_facts:
            text_parts.append(f"\n### 【Currently Valid Facts】(Simulation results original text)")
            for i, fact in enumerate(self.active_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")
        
        # Historical/expired facts (full output, untruncated)
...
class AgentInterview:
    ...
    def to_text(self) -> str:
        ...
        # Show full agent_bio, untruncated
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter junk content containing question numbers (Q1-9)
                skip = False
                for d in '123456789':
...
class ZepToolsService:
    ...
    2. panorama_search - Breadth search (retrieve full picture, including expired content)
    3. quick_search - Simple search (fast retrieval)
    4. interview_agents - In-depth interview (interview simulated agents, retrieve multi-perspective views)
    
    [Base Tools]
    - search_graph - Graph semantic search
    - get_all_nodes - Retrieve all graph nodes
    - get_all_edges - Retrieve all graph edges (with temporal info)
    - get_node_detail - Retrieve node details
    - get_node_edges - Retrieve node-related edges
    - get_entities_by_type - Retrieve entities by type
    - get_entity_summary - Retrieve entity relationship summary
    """
    ...
    def search_graph(
        ...
        Use hybrid search (semantic+BM25) to search for related info in graph.
    ...
    def _local_search(
        ...
        Retrieve all edges/nodes, then perform local keyword matching
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Retrieve all graph nodes (paginated)
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        """
        Retrieve a single node's details
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Retrieve all graph edges, then filter for edges related to the specified node
        ...
        Returns:
            Edge list
        """
        logger.info(f"Retrieving node {node_uuid[:8]}...  related edges")
    ...
    def get_entities_by_type(
        ...
        filtered = []
        for node in all_nodes:
            # Check if labels include specified type
            if entity_type in node.labels:
                filtered.append(node)
        
        logger.info(f"Found {len(filtered)}  {entity_type} type entities")
        return filtered
    ...
    def get_entity_summary(
        ...
        Search all info related to this entity and generate a summary
    ...
    def get_simulation_context(
        ...
        Comprehensively search all info related to simulation requirement
    ...
    def insight_forge(
        ...
        1. Use LLM to decompose question into sub-questions
        2. Perform semantic search for each sub-question
        3. Extract related entities and retrieve their details
        ...
        # Step 3: Extract related entity UUIDs from edges, only retrieve these entities (not all nodes)
        entity_uuids = set()
        ...
        for uuid in list(entity_uuids):  # Process all entities, no truncation
            if not uuid:
                continue
            try:
                # Retrieve each related node individually
    ...
    def _generate_sub_queries(
        ...
        Decompose a complex question into independently searchable sub-questions
        """
        system_prompt = """You are a professional question analysis expert. Your task is to decompose a complex question into sub-questions that can be independently observed in the simulation world.
...
Return a JSON-formatted sub-question list."""
        ...
        try:
            ...
            logger.warning(f"Sub-question generation failed: {str(e)},using default sub-questions")
            # Fallback: return variants of original query
            return [
                query,
    ...
    def interview_agents(
        ...
        Call real OASIS interview API to interview running simulation agents:
        1. Auto-read profile files to understand all simulation agents
        2. Use LLM to analyze interview needs and intelligently select relevant agents
        ...
        [IMPORTANT] This function requires simulation environment to be running (OASIS not closed)
        
        [Use Cases]
        - Need perspectives from different roles on events
        - Need to collect opinions and viewpoints from multiple parties
        - Need real responses from simulation agents (not LLM-simulated)
        ...
        try:
            ...
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # Using optimized prompt
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Retrieve interview results for this agent on both platforms
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(reddit_profile_path):
            try:
                ...
                logger.info(f"Loaded from reddit_profiles.json: {len(profiles)}  profiles")
                return profiles
            except Exception as e:
        ...
        if os.path.exists(twitter_profile_path):
            try:
                ...
                logger.info(f"Loaded from twitter_profiles.csv: {len(profiles)}  profiles")
                return profiles
            except Exception as e:
...
Please select up to {max_agents} most suitable agents for interview, and explain the reasoning."""
        ...
        try:
            ...
            # Retrieve full info for selected agents
            selected_agents = []
            valid_indices = []
...
6. Ask directly, do not include background or prefix
