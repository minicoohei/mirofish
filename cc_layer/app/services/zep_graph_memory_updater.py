...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert activities to text descriptions for Zep
        
        Use natural language format so Zep can extract entities and relationships
    ...
    def _describe_dislike_post(self) -> str:
        """Downvote post - includes original post content and author info"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_create_comment(self) -> str:
        ...
        if content:
            if post_content and post_author:
                return f"Commented \"{content}\" on {post_author}'s post: \"{post_content}\""
            elif post_content:
                return f"Commented \"{content}\" on post: \"{post_content}\""
            elif post_author:
                return f"Commented \"{content}\" on {post_author}'s post"
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation actions log file and updates new agent activities to Zep graph in real-time.
    Groups by platform, batch-sends to Zep after accumulating BATCH_SIZE activities.
    
    All meaningful actions are updated to Zep, action_args contain full context info:
    ...
    # Send interval (seconds), avoid too frequent requests
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-grouped activity buffers (each platform accumulates to BATCH_SIZE before batch send)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contain full context info (original post content, user names, etc.).
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Send interval, avoid too frequent requests
    ...
    def _flush_remaining(self):
        ...
        # Then send remaining buffer activities per platform (even if less than BATCH_SIZE)
        with self._buffer_lock:
